...
掲示板の掃除を「件数だけ」にしたとき（2026-08-22）も同じやり方です。
`all.sql` を計4回、エラー0・行数の変化0（profiles 1 / clubs 1 / club_members 1 / club_posts 2）。

**日数の削除（3日）を消したことは、それだけで落ちるように確かめました。**

| 入れたもの | `club_feed()` を呼んだ結果 |
|---|---|
| 105件（今日100件＋10日前5件） | **100件が残る**。あふれた古い5件だけ消える |
| **1年前の書き込みを3件だけ** | **3件とも残る**（前のコードなら0件） |
| 昨日の・満杯のお願い | 消える（今までどおり） |
| 今日の・満杯のお願い | 残る（「今日はもうお願いした」の判定に要るため） |
| 昨日の・まだ埋まっていないお願い | 残る |

2つ目が決め手です。件数の蓋しか無いことを確かめるには、**件数で消えない少なさ**で
試さないと意味がありません（105件の試験だけでは、日数で消えたのか件数で消えたのか
区別が付きません）。
...
オーナー・2026-09-26「ランクマッチは消してください」。Edge Function（`rated-tick`）・毎日の起動
（GitHub の cron）・デプロイの workflow・`rated_*` の関数とポリシーは repo から消しました。

**サーバーに残っているものを片付けるのはオーナーの操作です。**

1. `all.sql` を流す（先頭の `drop function` / `drop policy` で `rated_*` の関数とポリシーが落ちる）
2. Supabase ダッシュボード → **Edge Functions** → `rated-tick` を削除（repo からは上げ直さない）

`rated_*` の**表は残します**（`all.sql` はデータを消さない決まり）。RLS が入ったまま・
ポリシー無し・権限も取り上げてあるので、端末からは読めも書けもしません。
...
| 表の数 | ポリシーの数 | 関数の数 | RLSが無い表 |
|---|---|---|---|
| 25 | 33 | 61 | **0** |

**見るのは2つだけです。**

- **表の数 25**。足りなければ表が作られていない
- **RLSが無い表 0**。**0 以外なら危険**（誰でも読み書きできる表がある）

★**関数の数は環境で変わります。** 上の61はローカルの数（2026-09-26・ランクマッチの関数を落としたあと）で、ローカルは `pgcrypto` を
