# リファクタリング設計書 v2 — 「1本の太い幹」をドメイン別の枝に分解する（**完了**）

> ## ✅ この設計は**やり終わりました**（2026-08-18 に確認）
>
> 以下は 2026-08-11 時点の**計画**です。**いまのコードの説明ではありません。**
> 数字（`gameStore.ts` 9,566行・157アクション、点検 約50本、`SAVE_VERSION=41` など）は
> 全部その日のもので、**もう当たっていません**。実測は次のとおり。
>
> | | 計画を書いた日（8/11） | いま（8/18） |
> |---|---|---|
> | `src/store/gameStore.ts` | 9,566行・157アクション | **713行**（操作は `store/slices/` の9本へ） |
> | 成長の実装 | 自チームと CPU で2系統 | `engine/growth.ts` 1本（`growPlayer` / `applyGrowth`） |
> | persist | gameStore の中に ~860行 | `store/persistence/`（migrate / merge / normalize / saveVersion） |
> | `SAVE_VERSION` | 41 | **42** |
> | 点検 | 約50本 | **108本**（一覧は `scripts/run-checks.mjs`） |
>
> **読むのは「どういう考えで割ったか」だけにしてください。**
> いま何がどこにあるかは `CLAUDE.md` の表と `README.md` を見ること。
> ここに書いてある行番号（`L1269–2439` のような）は、どれも今のファイルには当たりません。
