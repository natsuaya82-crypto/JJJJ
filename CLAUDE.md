...
いまは **v2.0.8**（2026-09-16 にオーナーの指示で 2.0.7 から上げた・build 180 で提出）。
**オーナーがOKを出すまで次の版に上げないでください。**

★**版を上げるときは、出していないぶんのお知らせを新しい版へ移すこと。** 2.0.7 として
App Store へ出したのは build 177 ぶんで、そのあと 2.0.7 のまま積んだ build 178・179 ぶんは
2.0.7 のエントリに追記してありました。**そのまま版だけ上げると、新しい版のお知らせが空で、
遊ぶ人が受け取る中身は「出したはずの古い版」の側に並びます。**「配信し終えたエントリは
書き換えない」は**出したものの記録**を守る決まりなので、出していないぶんは移してよい
（消すのではなく動かす）。
