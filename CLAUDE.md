...
### 画面下から出るシートを勝手に作らないこと（`check-bottom-sheet`）

**オーナーが嫌いなUIです。増やしてよいのは、オーナーが「これはシートで」と
はっきり言ったときだけ。**
...
走友会の「入る前に中身を見る」を、頼まれてもいないのにシートで作って怒られました。
**形（シート／ページ／その場で開く）を決めるのはオーナーです。** 何も言われていない
ときは、いま画面にあるものをその場で開く・広げるのが既定。
