...
| `src/data/logoPresets.ts` の `remoteLogoId` | **サーバーから読んだ「他の人」のロゴをどれにするか**。フレンド一覧・申請/承認・ランクマッチの順位表と「あなたの部屋」が全部これを通る。`logo_01` を素通りさせないのは、まだ新しいアプリでログインしていない人が `logo_01` のまま保存されていて、そのまま出すと全員が同じ鶴になるため。★**`TeamLogoSVG` に `teamId` としてサーバーのユーザーIDを渡さないこと**——`teamId` は**自分のセーブの中のチームID**を渡す口で、`logoId` が無いと `s.teams.find(t => t.id === teamId)` を引く。UUID はそこに居ないので必ず外れ、最後の逃げ道の**ハッシュで作った紋章**に落ちる。サーバーは `logoId` を返していて型（`RatedRow.logoId`）にも入っていたのに、**取ってきて捨てていた**（オーナー・2026-08-22「ここなんでアイコン適当なの？」）。同じ人がフレンド一覧と順位表で別の絵になる。`check-team-logo` が見張る（式が1本か・口の数と通っている数・`teamId={…userId}` が無いか・相手を出す画面が `logoId` を渡すか）。★**まだ通っていないものが2つあります**——掲示板の書き込み（`clubsApi`）とブロックした人の一覧（`moderationApi`）が `r.logo_id || 'logo_01'` のまま。症状が違う（全員同じ鶴）ので別に判断する |
