...
| `src/utils/notifItems.ts` | 通知の中身の収集（ベルの数字と通知ページの内容を揃える）。★**`teams`（国内52）に居るかで札を絞らないこと**——トレードの打診は海外からも来るので、**海外のぶんだけベルに出ない**形になっていた（2026-09-16）。クラブが実在するかは選手の所属で分かる。そして、**プレゼントの中身の書き方**（`giftContents`）。以前は画面に直書きで `jewels ? 'ジュエル◯個' : 'カード◯枚'` の**2択**だったので、優勝トロフィーのプレゼントが**「カード0枚」**と出ていた（オーナー・2026-08-20）。中身を増やしたらここだけ直すこと |
...
`teams`（国内52）に閉じて**いた時期があり、海外の選手は移籍市場に一度も並ばず、
...
（オーナー・2026-09-16「二も一緒」）。`foreignLeagues` を渡し忘れると国内に閉じます。
