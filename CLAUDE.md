...
| `src/utils/eventTime.ts` の `EVENT_LABEL` / `eventDistKey` | **記録会の距離の呼び名とキー**。距離（5000/10000/21097/42195）で引く形とキー（`d5000`…）で引く形の**2通り**に割れて10か所に写っていた。★ニュースの見出しだけ「ハーフマラソン」で、こちらは「ハーフ」です（`docs/BACKLOG.md` A-新2・未決） |
