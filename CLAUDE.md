...
| `src/utils/myPlayer.ts` | **自分でつくる選手の決まり**。`MY_PLAYER_STAT_MIN`(60) / `MY_PLAYER_STAT_MAX` / `myPlayerBlockReason` と、**振り分けポイント**。★**下限が要る理由**——捨てた能力はカードで育て直せるので、下限が無いと「5本を99で始める」形が通ります（オーナー・2026-08-21「カードで合成でバケモンが完成してしまう」）。★**育て切ったときの上限は `STAT_CAP`(99) 1本で、どの能力も99まで伸びます。** **「育て切ると全能力の平均が92（合計644）」は廃止しました**（オーナー・2026-08-22「その平均92をやめろ」）——合計を644に固定して低い能力から水を張る形だったので、**どう振っても到達点が同じ（OVR92）**でした。振り分けが決めるのは**どこから始めるか**だけです。★この92は実装のとき（`2c008b3`・2026-07-23）に**確認せずこちらで決めた数字**でした。バランスの数字を勝手に置かないこと。★**上限の式を2つ持たないこと**——同じ水割りが store と画面（`CreateMyPlayerPage`）の2か所に写してあり、片方だけ直すと「画面の試算と実際の伸びしろが違う」になります（`check-my-player` の⑤が両方を見る）。★**振り分けポイントは出どころで違う**——`MY_PLAYER_POINTS_INITIAL`(500)＝新規作成の記念／`MY_PLAYER_POINTS_GRANT`(560)＝記念の配布ぶん。**回数だけ持たないこと**（額をどこかの分岐で当てることになる）。配るときに額を決めて `playerCreateGrants` へ入れ、使う側は先頭を取る。★**画面の「押せるか」と store の「受け付けるか」を同じ関数から出すこと**（`bidGate` と同じ形）|
