...
セーブは `src/store/persistence/` に分かれています。`docs/REFACTORING_DESIGN.md` は
**そのときの計画の記録**で、いまのコードの説明ではありません（行番号も本数も当たりません）。
**いま何がどこにあるかは、下の表と `README.md` が正です。**

**`docs/` に置いてある md は、それぞれ頭に「いつの話か」が書いてあります。**
実装前の設計・草案・監査のメモが混ざっているので、**直す場所を探すときは md ではなく
`CLAUDE.md` の表とコードを見ること**（古い md を読んで、もう無いファイルを直しに行った例が
実際にあります）。`npm run check` の `docs` が、md が指しているファイルが本当にあるかを見張ります。
