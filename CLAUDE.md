...
| `src/utils/notifItems.ts` | 通知の中身の収集（ベルの数字と通知ページの内容を揃える）と、**プレゼントの中身の書き方**（`giftContents`）。以前は画面に直書きで `jewels ? 'ジュエル◯個' : 'カード◯枚'` の**2択**だったので、優勝トロフィーのプレゼントが**「カード0枚」**と出ていた（オーナー・2026-08-20）。中身を増やしたらここだけ直すこと |
