...
**バージョンは v2.0.4 で固定です。オーナーがOKを出すまで 2.0.5 に上げないでください。**
（2026-08-16 にオーナーの指示で 2.0.3 → 2.0.4）

実機確認のあいだは、中身を足してもバージョンは上げません。ビルド番号だけを上げて
2.0.4 のまま出し続けます。お知らせも新しいエントリを作らず、v2.0.4 のエントリに追記します。

| 何 | どこが正 | 誰が書く |
|---|---|---|
| バージョン（2.0.4） | `src/data/appMeta.ts` の `APP_VERSION` | `npm run sync:version` と CI |
