...
const CHECKS = [
  ...
  'race-teams',
  // 中継の時計（タスキをつないで途切れずに走る）。棒グラフも 2.0.9 の3Dもこの1本だけを読む
  'race-timeline',
  ...
  // トレードの相手は231クラブ（海外が相手でもお金は両側で動き、指名権は消えない）
  { name: 'trade-world', shim: true },
  ...
  'market-list',
  // 画面とサーバー（all.sql）の2か所にある上限が食い違っていないか
  // （対戦を始められる最少チーム数・走友会の人数の上限）
  'server-caps',
  'nationality',
  ...
  'roster-fill',
  // 相手のロゴが remoteLogoId 1本を通っているか。当時のランクマッチの順位表だけが
  // logoId を渡さず teamId={r.userId}（＝サーバーのUUID）を渡していて、
  // 引けるはずのない探し方をした結果ハッシュの紋章が出ていた
  // （オーナー・2026-08-22「ここなんでアイコン適当なの？」）
  'team-logo',
  // md がいまのコードを指しているか。**コードだけが先に進む**ので、放っておくと
  // 「もう無いファイルを直しに行く」が起きる（オーナー・2026-08-18
  // 「古いmd系とか全部新しくしないと直すのが古い場所とかになっちゃうよ」）
  'docs',
  // ストアの「最新情報」＝いまの版の CHANGELOG の本文。Apple が断る文（長さ・記号）を
  // お知らせを書いた時点で落とす（提出は scripts/store-submit.ts が同じ本文を送る）
  'store-text',
]
