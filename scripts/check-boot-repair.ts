...
check('2回目の起動で点が増えない（冪等）',
  leagueStandingRows(s2, divisionLeagueId(3)).find(r => r.teamId === pickedId)?.totalPoints === expected)
...
// （いまの部なので昇降格したあとの年に当てはめると記録が動く）。走った結果だけが手がかり。
{
  ...
  const past = {
    year: 2027,
    leagues: divisionLeagues({
      1: [{ id: 'd1-0', name: '1部戦', results: undefined }] as never,
      2: [{ id: 'd2-0', name: '2部戦', results: undefined }] as never,
      // 自分は3部の日程を走った（結果入り）
      3: races.map(r => ({ ...r, id: `d3-${r.id}` })) as never,
    }, {
  ...
  check('  2部の側から消えている（両方に居ない）',
    !leagueStandingRows(fixed, divisionLeagueId(2)).some(r => r.teamId === pickedId))
  const again = repairLoadedSave(out as never)
  check('  2回目は直すものが無い（冪等）', again.repairs.length === 0, again.repairs.join(' / '))
}
