...
console.log('\n[1] 移籍金の年齢倍率（段をやめて折れ線に）')
// ★2026-08-11 に**段をやめました**（オーナー判断・docs/BACKLOG.md A-1）。
//   旧：〜22歳×5／23〜27×4／28〜31×3／32〜×2 の4段。能力が1も変わっていないのに
//       誕生日で −20%／−25%／−33% と落ちるので「27歳の主力は28歳になる前に売る」が最適解だった。
//   新：もとの4段の**帯の中央**を通す折れ線（21歳=5／25歳=4／29.5歳=3／34歳=2）。
//       帯ごとの平均はほぼそのまま（全5,800人の移籍金の合計で +0.1%）。
//
//   ここで見るのは「段が無いこと」と「向きと端が正しいこと」。
//   **1年あたりの下落幅に線を引く**（段が戻ってきたら必ず落ちる）。
{
  const v = (o: number, a: number) => calcTransferValue(P(o, a))
  const ratio = (a: number, b: number) => v(90, a) / v(90, b)
  const near = (got: number, want: number, tol = 0.01) => Math.abs(got - want) <= tol

  // ── 段が無い ──
  // 1年で1割を超えて落ちる年齢が1つでもあれば、それは段が戻ったということ。
  // 丸め（百万円単位）のぶんだけ緩めて 12% で見る
  const drops: string[] = []
  for (let a = 19; a <= 40; a++) {
    const d = 1 - v(90, a) / v(90, a - 1)
    if (d > 0.12) drops.push(`${a - 1}→${a}歳 -${(d * 100).toFixed(0)}%`)
  }
  check('誕生日で1割を超えて落ちる年齢が無い（段が戻っていない）', drops.length === 0, drops.join(' , '))

  // ── 端 ──
  check('21歳以下は据え置き（×5の帯の上端）', v(90, 18) === v(90, 21) && v(90, 20) === v(90, 21))
  check('34歳以降は据え置き（×2の下限）', v(90, 34) === v(90, 38) && v(90, 36) === v(90, 34))

  // ── もとの帯の中央は変わっていない（値段の水準を動かしていないことの確認）──
  check('21歳:25歳 = 5:4', near(ratio(21, 25), 5 / 4))
  check('25歳:34歳 = 4:2', near(ratio(25, 34), 2))
  check('29歳あたりで×3（旧28〜31の帯の中央）', near(ratio(21, 30), 5 / 2.889, 0.02), ratio(21, 30).toFixed(3))

  // ── 向き ──
  const monotone: string[] = []
  for (let a = 19; a <= 40; a++) if (v(90, a) > v(90, a - 1)) monotone.push(`${a}歳`)
  check('年を取って上がる年齢が無い', monotone.length === 0, monotone.join(','))
  check('ピーク前(26歳)は29歳より高い', v(90, 26) > v(90, 29))
  check('20歳の上乗せは34歳の2.5倍（若さの上乗せが消えていない）',
    near(ratio(20, 34), 2.5), ratio(20, 34).toFixed(3))
