...
console.log('\n⑥ 押すボタンを画面で手書きしていない（今日より増えていない）')
{
  ...
  //
  // ★**`rgba(...)` も見ること**（2026-08-15）。16進と `\${}` しか見ていなかったので
  //   `0 3px 0 rgba(0,0,0,0.45)` と書いた**21件が丸ごと網の外**にいて、
  //   fixture は「0件」だった＝**この網は何も守っていなかった。**
  //
  // ★**`inset` は数えないこと。** `inset 0 1px 0 rgba(255,255,255,0.22)` は
  //   ガラスの配合そのもの（`tokens.ts` の `glassStyle`）で、飴玉ではない。
  //   区別せずに数えたら49件になり、そのうち28件がこれだった。
  //   飴玉は**ぼかし無しで下へ落ちる影**なので、2px 以上だけを見る
  //   （`0 1px 0` は境目の線として使うことがある）。
  const SLAB = /(?<!inset )\b0 [2-9]px 0 (?:#[0-9a-fA-F]{3,6}|\$\{|rgba?\()/g
  const counts: Record<string, number> = {}
  for (const f of screens) {
...
console.log('\n⑮ 画面に絵文字を書いていない')
{
  // ★このアプリのアイコンは**全部インラインSVG**です。絵文字を混ぜるとそこだけ
  //   浮きます（OSごとに絵柄も大きさも違う）。2026-08-20 に優勝トロフィーを
  //   🏆 で2か所へ直書きして、実機で見たオーナーに指摘されました
  //   （「どうして既存の絵文字でやったか言えよ」）。リポジトリ全体で絵文字は
  //   その2件だけ＝作法を確認せずに貼った、という形です。
  // ★**コメントを外してから見ること。** 外さないと、この経緯の説明文自体に当たります。
  // ★見るのは**カラー絵文字だけ**。★ ☆ ✓ ♡ ¥ のような記号は昔から使っていて、
  //   これは絵ではなく文字なので混ぜないこと（U+2600〜27BF まで網に入れると
  //   チェックマークや星が22件当たって、本物の絵文字が埋もれます）。
