...
import type { GameState, Division, Player, Team, RaceResults, TransferListing, IncomingOffer, TradeNegotiation, ContractRequest, AcquisitionOffer, TeamRole, ForeignCategory, FacilityKey, CardRarity, CardStatKey, TrainingCard, Ratings, Race, TransferRecord, SeasonAward, Nationality, Specialty, SeasonStanding, ExpiredNegKind, GmOffer } from '../types'
...
import { ovr, retirementAgeOf, faMarketSalary, salaryAppealBonus, seasonPerfProfile, playerConsentToMove, freeContactConsent, keyPlayerStatus, calcTransferValue, racesConsumed, packForeignApps } from '../utils/playerUtils'
import { strHash } from '../utils/hash'
import { withMorale, setMorale, MORALE_DEFAULT } from '../utils/condition'
...
import { appraiseMove, buildDestination, rankOffers, regionOfLeague, hasNoPlayingTime, seeksPlayingTime, type Destination, type Appraisal } from '../utils/transferDecision'
import { isOwnedBy, eligibilityCtx, canBePoached, canListForSale, canLoanOut, canTradeAway, canAcceptOfferFor, isLeavingClub } from '../utils/transferEligibility'
import { contractTalkCtx, effectiveDemandSalary, canOfferRenewal, canRequestRenewal, canReNegotiate, liveContractOf, hasContractTalk, MAX_CONTRACT_ROUNDS } from '../utils/contractTalk'
// トレードの釣り合いの判断（下限・上限・主力割増・OVR差）は tradeValue.ts の1箇所
import { tradeValues, faceValueOf, tradeBalance, tradeNotLopsided, TRADE_MIN_RATIO, TRADE_OK_RATIO, TRADE_HARD_NO_RATIO } from '../utils/tradeValue'
import type { TradeValueCtx } from '../utils/tradeValue'
import { findClub, domesticTeamIdSet as domesticTeamIdSet_, allForeignClubs, foreignClubIdSet, leagueOfClub, bigClub } from '../utils/clubs'
// 殿堂入りチーム（登録時の数値で固定）
...
import { tierBudget, tierOf, tierOfClubId, tierStrength, isBigClub, isStepUp, MAJOR_NEWS_OVR, tierOfPlayerClub, tierFromDomesticRank, tierFromForeignRank, allTieredClubs } from '../utils/clubTier'
