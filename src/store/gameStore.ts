...
import { clubLabel, transferHeadline, awardHeadline, retirementHeadline, divisionChampionHeadline, loanHeadline, seekPlayingTimeHeadline, injuryHeadline, signedWithFeeHeadline, soldPlayerHeadline, joinedHeadline, renewalHeadline, loanInOutHeadline, segmentPrizeHeadline, overseasMoveHeadline, foreignSignedHeadline, freeTransferHeadline, cpuSignedHeadline, loanReplyHeadline, raceWinnerHeadline, myFinishHeadline, segmentWinHeadline, boardEvalHeadline, rivalHeadline, segmentRecordHeadline, worldChampFinishHeadline, recordHeadline, divisionMoveHeadline, seasonOpenHeadline, divisionsFoundedHeadline, massFreeAgentHeadline, growthHeadline, retiredHeadline, bonusPayoutHeadline, sponsorEndHeadline, objectiveBonusHeadline, seasonBudgetHeadline, deficitPickPenaltyHeadline, tradeAcceptedHeadline, tradeSummaryHeadline, dynastyHeadlines, type NewsItem } from '../utils/newsItems'
...
import type { GameState, Division, Player, Team, RaceResults, TransferListing, IncomingOffer, LoanResponse, TradeNegotiation, ContractRequest, AcquisitionOffer, AITradeOffer, TeamRole, ForeignCategory, FacilityKey, CardRarity, CardStatKey, TrainingCard, Ratings, Race, TransferRecord, SeasonAward, Nationality, Specialty, SeasonStanding, ExpiredNegotiation, ExpiredNegKind, GmOffer } from '../types'
...
import { generateDraftPool, buildDraftOrder, generateForeignLeaguePlayers, refreshForeignLeagues, nationalityToForeignCategory } from '../engine/playerGenerator'
import { simulateRace, buildCpuLineups } from '../engine/raceEngine'
...
import { saleAnswers, withSaleAnswer, keepSaleAnswers } from '../utils/saleAnswer'
// 選手がクラブを移るときの後始末は movePlayer.ts に集約（所属・名簿・移籍金・履歴・レンタル）
import type { DepartureNotice } from '../utils/movePlayer'
import { movePlayer } from '../utils/movePlayer'
import { appraiseMove, buildDestination, rankOffers, dreamRegionOf, regionOfLeague, hasNoPlayingTime, seeksPlayingTime, type Destination, type Appraisal } from '../utils/transferDecision'
import { isOwnedBy, eligibilityCtx, canBePoached, canListForSale, canLoanOut, canTradeAway, canAcceptOfferFor, canWishTransfer, isLeavingClub } from '../utils/transferEligibility'
import { contractTalkCtx, effectiveDemandSalary, canOfferRenewal, canRequestRenewal, canReNegotiate, isLiveContract, liveContractOf, hasContractTalk, MAX_CONTRACT_ROUNDS } from '../utils/contractTalk'
...
import { rankedStandings, rankOfTeam, seasonDivisionStandings, divisionStandings, domesticThroughRankOfTeam, newSeasonStandings, syncSeasonStandings, divisionOf, teamsInDivision, joinsDraft, domesticThroughRank, segmentPrizeByTeam, DIVISIONS, DIVISION_SIZE, PROMOTION_SLOTS, TOP_DIVISION } from '../utils/league'
...
import { ACHIEVEMENT_JEWELS, checkRaceAchievements, checkSeasonAchievements, podiumJewels, selectSeasonObjectives } from '../engine/achievements'
