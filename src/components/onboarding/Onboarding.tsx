    ...
    <div style={{
      ...
      {step === 'welcome' && (
        <div style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: '32px 24px', textAlign: 'center' }}>
          ...
          <div style={{ fontSize: F.label, color: '#5C5870', letterSpacing: '4px', marginBottom: '8px' }}>
          ...
          <div style={{ fontSize: F.bodyLg, color: '#9B97A8', marginBottom: '48px', lineHeight: 1.6, maxWidth: '280px' }}>
          ...
          <button onClick={() => setStep('team_select')} style={{
            ...
            color: '#0A0912', fontSize: F.title, fontWeight: '900',
      ...
      {step === 'team_select' && (
        <div style={{ flex: 1, width: '100%', display: 'flex', flexDirection: 'column', minHeight: 0 }}>
          <div style={{ padding: '24px 20px 12px', flexShrink: 0 }}>
            <div style={{ fontSize: F.caption, color: '#5C5870', letterSpacing: '3px', marginBottom: '6px' }}>STEP 1 / 2</div>
            <div style={{ fontSize: F.headLg, fontWeight: '900', color: '#F0EDE8' }}>チームを選択</div>
            <div style={{ fontSize: F.body, color: '#9B97A8', marginTop: '4px' }}>率いるチームを選んでください</div>
            <div style={{ fontSize: F.label, color: '#C9A84C', marginTop: '6px' }}>どのチームを選んでも JPEL 3部からのスタートです</div>
          </div>
            ...
            {(['北海道・東北', '関東', '中部', '関西', '中国・四国', '九州・沖縄'] as const).map(region => {
              ...
              return (
                <div key={region} style={{ marginBottom: '18px' }}>
                  <div style={{ fontSize: F.caption, color: '#5C5870', letterSpacing: '2px', padding: '0 2px', marginBottom: '8px' }}>
                    {region}
                  </div>
                  {regionTeams.map(team => {
                    ...
                    return (
                      ...
                      >
                        ...
                        <div style={{ flex: 1 }}>
                          <div style={{ fontSize: F.sub, fontWeight: '700', color: '#F0EDE8' }}>{team.name}</div>
                          <div style={{ fontSize: F.caption, color: '#5C5870', marginTop: '1px' }}>{team.city}</div>
                        </div>
                        {selected && (
                          <div style={{
      ...
      {step === 'customize' && (
        <div style={{ flex: 1, width: '100%', overflowY: 'auto', minHeight: 0, boxSizing: 'border-box', padding: `0 20px calc(${adH}px + env(safe-area-inset-bottom) + 24px)` }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '4px', padding: '12px 0 20px' }}>
            ...
            <div>
              <div style={{ fontSize: F.caption, color: '#5C5870', letterSpacing: '3px' }}>STEP 2 / 2</div>
              <div style={{ fontSize: F.head, fontWeight: '900', color: '#F0EDE8', lineHeight: 1.2 }}>チームをカスタマイズ</div>
            </div>
          </div>
          ...
          <div style={{
            ...
            <div>
              ...
              <div style={{ fontSize: F.title, fontWeight: '800', color: '#F0EDE8' }}>{teamName || selectedTeam.name}</div>
            </div>
          </div>

          {/* Logo select */}
          <div style={{ marginBottom: '20px' }}>
            <label style={{ fontSize: F.label, color: '#9B97A8', letterSpacing: '2px', display: 'block', marginBottom: '8px' }}>
              チームロゴ
            </label>
            <button
              type="button"
            ...
            >
              ...
              <span style={{ flex: 1, textAlign: 'left', fontSize: F.bodyLg, fontWeight: 700, color: '#F0EDE8' }}>変更する</span>
          ...
          <div style={{ marginBottom: '20px' }}>
            ...
            <input
              ...
              style={{
                ...
                backgroundColor: '#1E1B2E', color: '#F0EDE8', fontSize: F.title,
          ...
          {nameError && (
            <div style={{ fontSize: F.body, color: '#E8462A', marginBottom: '12px', padding: '8px 12px',backgroundColor: '#E8462A15' }}>
              {nameError}
            </div>
          )}

          <div style={{ fontSize: F.label, color: '#5C5870', marginBottom: '32px', padding: '10px 12px',backgroundColor: '#1A1828', border: '1px solid #2E2B42' }}>
