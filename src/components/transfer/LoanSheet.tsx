  ...
  return createPortal((
    <div onClick={onClose} style={{ position: 'fixed', inset: 0, zIndex: 300, background: 'rgba(0,0,0,0.6)', display: 'flex', flexDirection: 'column', justifyContent: 'flex-end' }}>
      <div className="sheet-up" onClick={e => e.stopPropagation()} style={{ width: '100%', maxWidth: 480, margin: '0 auto', maxHeight: '85vh', overflowY: 'auto', background: C.surface,border: `1px solid ${C.border2}`, borderBottom: 'none', boxShadow: '0 -12px 40px rgba(0,0,0,0.6)', paddingTop: 8, paddingLeft: 16, paddingRight: 16, paddingBottom: `calc(16px + env(safe-area-inset-bottom) + ${adH + 50}px)` }}>
        ...
        <div style={{ fontSize: 13, fontWeight: 800, color: C.text, marginBottom: 4 }}>{player.name} をレンタル</div>
        <div style={{ fontSize: 10, color: C.textDim, marginBottom: 14, fontFamily: SAIRA }}>買わずに借りる（レンタル枠 {slots}/3・移籍金なし・給与は自チーム負担）。期間を選んで要請（次レースで回答）。</div>
        {pending ? (
          <div style={{ fontSize: 13, color: C.blue, fontWeight: 700, textAlign: 'center', padding: 12 }}>レンタル要請中 — 次レースで回答</div>
        ) : full ? (
          <div style={{ fontSize: 13, color: C.red, fontWeight: 700, textAlign: 'center', padding: 12 }}>レンタル枠が満杯です（3/3）</div>
        ) : (
          <div style={{ display: 'flex', gap: 10 }}>
            {[1, 2].map(y => (
              <button key={y} onClick={() => onSubmit(y)}
                style={{ flex: 1, padding: '14px',border: `1.5px solid ${alpha(C.blue, 0.5)}`, background: alpha(C.blue, 0.12), color: C.blue, fontSize: 15, fontWeight: 800, cursor: 'pointer', fontFamily: SAIRA }}>
                {y}年契約
              </button>
            ))}
          </div>
        )}
        <button onClick={onClose} style={{ display: 'block', width: '100%', marginTop: 12, padding: '13px',border: `1px solid ${C.border}`, background: C.surface2, color: C.textDim, fontSize: 14, fontWeight: 800, fontFamily: 'inherit', cursor: 'pointer' }}>キャンセル</button>
      </div>
    </div>
  ), document.body)
}
