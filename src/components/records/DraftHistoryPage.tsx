            ...
            {list.map((p, i) => {
              ...
              return (
                <div key={p.id} {...lp(p.id)} style={{ display: 'flex', alignItems: 'center', gap: '10px', padding: '7px 0', borderBottom: i < list.length - 1 ? `1px solid ${C.border}` : 'none', cursor: 'pointer', opacity: isRetired ? 0.6 : 1 }}>
                  <span style={{ fontFamily: SAIRA, fontSize: F.subLg, fontWeight: 900, color: overall === 1 ? C.gold : overall <= 3 ? C.green : C.textSub, width: '32px', textAlign: 'center', flexShrink: 0 }}>{overall}<span style={{ fontSize: F.micro, color: C.textDim, fontWeight: 700 }}>位</span></span>
                  ...
                  <div style={{ flex: 1, minWidth: 0 }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 5 }}>
                      <span style={{ fontFamily: SAIRA, fontSize: F.bodyLg, color: C.text }}>{p.name}</span>
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', gap: 4, marginTop: 1, minWidth: 0 }}>
                      ...
                      <span style={{ fontSize: F.tiny, color: isMine ? C.gold : C.textDim, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                        {team?.name ?? (isRetired ? '引退' : '未所属')} / {SPECIALTY_LABELS[p.specialty]} / {p.age}歳
                      </span>
                    </div>
                  </div>
                  <span style={{ fontFamily: SAIRA, fontSize: F.titleLg, fontWeight: '900', color: ratingColor(o) }}>{o}</span>
                </div>
              )
            })}
          </CardPanel>
  ...
  return (
    <div style={{ fontFamily: SAIRA, paddingBottom: '80px', minHeight: '100dvh' }}>
      ...
      <div style={{ padding: '0 16px' }}>
        <div style={{ fontSize: F.label, color: C.textDim, marginBottom: '14px' }}>年度を選ぶとその年のドラフト（1〜40位）を表示</div>
      </div>

      <div style={{ padding: '0 16px', display: 'flex', flexDirection: 'column', gap: '8px' }}>
        {years.length === 0 ? (
          <CardPanel>
            <div style={{ textAlign: 'center', color: C.textDim, fontSize: F.bodyLg, padding: '20px 0' }}>まだドラフト指名がありません</div>
          </CardPanel>
        ) : years.map(y => {
          ...
          return (
            <GlassButton key={y} full style={{
              ...
              <span style={{ fontFamily: SAIRA, fontSize: F.headLg, fontWeight: 900, color: C.gold, lineHeight: 1 }}>{y}<span style={{ fontSize: F.label, color: C.textDim, fontWeight: 700, marginLeft: 2 }}>年度</span></span>
              <span style={{ fontFamily: SAIRA, fontSize: F.label, color: C.textDim, padding: '2px 8px',background: alpha(C.gold, 0.12) }}>{count}名</span>
              <span style={{ marginLeft: 'auto', color: C.textGhost, fontSize: F.titleLg }}>›</span>
            </GlassButton>
          )
        })}
      </div>
