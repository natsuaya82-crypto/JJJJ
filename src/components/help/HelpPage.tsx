...
// ★遊び方に出す数は、実際に効いている定数から出すこと（画面に数字を書かない）。
...
/** 国内の一番上の格と一番下の格で、クラブ予算が何倍ちがうか */
const DOMESTIC_BUDGET_RATIO = Math.round(TIER_BUDGET[DOMESTIC_TOP_TIER] / TIER_BUDGET[DOMESTIC_BOTTOM_TIER])
...
const SECTIONS: Section[] = [
  {
    ...
    lines: [
      ...
      `JPELは1部・2部・3部の全${DOMESTIC_CLUB_COUNT}チーム。昇格を重ねて1部の頂点を目指します。`,
  ...
  },
  {
    ...
    lines: [
      ...
      `ドラフトで新人を指名し、ロスター（最大${ROSTER_MAX}名）と各区間の走者を決めます。1軍・2軍の区分はありません。`,
      '就任1年目だけは例外です。ドラフトは見学のみで指名に参加せず、代わりに選手を1人つくって加入させます。',
      `チームは全${DOMESTIC_CLUB_COUNT}クラブから選べますが、どのチームを選んでもスタートは3部です。昇格を重ねて1部の頂点を目指します。`,
  ...
  },
  {
    ...
    lines: [
      ...
      `シーズン終了後、各部の上位${PROMOTION_SLOTS}チームが昇格・下位${PROMOTION_SLOTS}チームが降格します（プレーオフはありません）。`,
  ...
  },
  {
    ...
    lines: [
      ...
      `ロスターは最大${ROSTER_MAX}名・最低${ROSTER_MIN}名。上限を超える契約や、下限を割る放出・解雇はできません。`,
      '契約期間が残る選手を解雇すると解約金がかかります（満了間近なら無償）。',
      '選手が移籍先を選ぶ基準は「そこで出番があるか」と「今より上のクラブか」。優勝争い・ECL出場・憧れの地域も見ます。',
      '強いクラブからの誘いでも、そこで出場機会が見込めないなら断ります。逆に出番を求めて格下へ移ることもあります。',
    ],
  },
  {
    no: '08',
    ...
    lines: [
      ...
      `自分の選手には最大${MAX_OFFERS_PER_PLAYER}クラブが手を挙げます。チャットに全部並び、本人の希望も聞けます。移籍金の逆提示は全クラブへ一斉に出せます。`,
  ...
  },
  {
    ...
    lines: [
      ...
      `クラブ予算はクラブの規模で決まり、規模は前年の国内順位で毎年上下します。1部優勝と3部最下位では${DOMESTIC_BUDGET_RATIO}倍の差がつきます。`,
