...
export function runTransferMarket(
  ...
  //      `teamRaces` が 0 なので `appraiseMove` の「いま走れているか」
  //      （いまの `transferDecision.playingStatus`）が
  //      **常に不発**になり、オーナー指示（2026-08-14「格下げてまでエースに
  //      なりたいやついないだろ。海外でやってる久保がいきなりJ3に移籍するか？」）で
  //      入れた関門 `tooFarDown` が**世界中で一度も発火していませんでした**。
  //      同じ理由で「主力として起用されており移籍を望んでいない」（`playingStatus`）も
  ...
  const sellCandidatesOf = (sellClub: MarketClub): SellCandidate[] => {
    ...
    return sellRoster
      ...
      // ★加入したときの契約が続いている選手は出せない（`isTransferLocked`。レンタルは別）。
