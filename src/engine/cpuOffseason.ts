...
//
// ■オフの一括処理はありません（ドラフトの直前の1回も、ほかと同じ `CPU_TICK_TRANSFERS` の1回）
//   以前はここに「オフの一括処理は残します」と書いてあったが、上限なしで回していたその1回が
//   413件 対 39件の塊になっていたので揃えた（CLAUDE.md「オフシーズンという考えはありません」）。
//   `needsPlayer` が門番なので、**シーズン中に埋まった穴はドラフト前の回でも埋めません**。
