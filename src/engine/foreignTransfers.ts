...
export function simulateCrossBorderTransfers<T extends Team>(params: {
  ...
  for (const t of cpuTeams) { jpnRoster[t.id] = [...(t.roster?.main ?? [])]; budget[t.id] = t.finance?.budget ?? 0 }
  ...
  for (let i = 0; i < N_IN; i++) {
    const buyers = cpuTeams.filter(t => jpnRoster[t.id].length < ROSTER_MAX && budget[t.id] >= MIN_BUY_BUDGET)
