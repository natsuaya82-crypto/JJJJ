...
export function runBackgroundRace(o: {
   ...
   * 代表チームの大会（世界選手権・大陸予選）は渡さない（中立）。
   * 渡し忘れると**施設が誰にも効かず、区間記録が偏る**。
   */
  ...
  const players = withFacilityBoost(o.players, o.clubs ?? [])
  const results = simulateRace(o.race, lineups, o.clubs ?? [], players, o.seasonProgress)
