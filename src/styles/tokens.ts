...
/**
 * 選手カード／クラブカードの色（`PlayerSheet` と `TeamDetailPage`）。
 *
 * この2画面だけ、アプリ全体（`C`）とは別の**紫がかった暗い色の組**で作ってあります。
 * 「あえて変えているものを無理に変えなくていい」（オーナー・2026-08-13）なので
 * 見た目はそのままにして、**値の置き場所だけ1本にしました**。
 *
 * 2026-08-18 に数えたら、この組が**2画面に240件そのまま手書き**されていました。
 * `C` と同じ段（背景4段・枠2段・文字3段・金3段）が丸ごともう1組あるのに名前が無いので、
 * 「金を少し明るくしたい」と思っても、どれを触ればいいのか誰にも分かりません。
 * 実際に `C` のほうでは、名前が無かったせいで金が3種類・水色が2種類に割れています。
 *
 * ★**この値を画面に直接書かないこと。** `check-ui-tokens` の⑪が見張ります。
 * ★アプリ全体の色へ寄せる（＝この2画面を青くする）かどうかはオーナー判断で、まだ聞いていません。
 */
export const CARD = {
  bg0:       '#0B0A12',   // いちばん暗い底
  bg:        '#0E0D17',
...
export const C = {
  ...
  // ── 名前が付いていなかった色（2026-08-18）──────────────────────
  // どれも「`C.cyan` にほぼ同じ色が2つある」ように見えていたが、数えたら
  // **意味が違う別の色**だった。寄せると意味が壊れる（ジュエルの色と汎用の
  // 水色が同じになる）ので、寄せずに名前を付けた。**値は変えていない。**
  jewel:     '#6dd5fa',  // ジュエル（課金通貨）。9画面で使う。C.cyan とは別（差21）
  amber:     '#F59E0B',  // カード強化の当たり演出。C.orange とは別（差27）
  outline:   '#061224',  // 文字の縁取り（textShadow を4方向に置くやつ）
  tileShadow:'#1a2050',  // ハブのメニュー札の落ち影（記録室・チーム・移籍）
  shareDim:  '#8C93A5',  // 共有カードの補足の文字（ShareCard / GmShareCard）
} as const
