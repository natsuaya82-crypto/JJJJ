...
/**
 * **開幕したとき、どのクラブにも居る人数**（全232クラブ・自チームも）。
 * これに満たないクラブは、開幕の直前（`startRegularSeason`）に `fillRostersForSeason` が
 * ここまで埋めます（オーナー・2026-09-25「チーム人数が開幕できないのを防ぐために
 * 20人以下の場合は20人になるまで自動補填」）。
 *
 * ★`ROSTER_MIN`(15) とは**役目が違うので別の数**です。あちらは**シーズン中に自分で
 *   減らす操作**（放出・解雇・売却）を止める線で、こちらは**開幕の時点で居る人数**。
 *   寄せると、開幕のたびに20人へ埋まるのに操作は20人で止まる（＝一度も放出できない
 *   クラブが出る）か、救済が15人までしか埋めない（＝オーナーの決定と違う）のどちらかになります。
 *
 * ★**人数で開幕を止めないこと**（`utils/seasonStart`）。止めると、この救済に届きません。
 * ★救済は毎年の入口（ドラフト・海外の補充・国内2・3部の若手）の**代わりではありません**。
 *   床だけにすると、毎年入る人数が「床を割った数」になり、FAが尽きて世界の上位が痩せます
 *   （2026-09-25 に `scripts/measure-roster-fill.ts` で実測）。
 */
export const SEASON_START_ROSTER = 20
...
/**
 * **「そのクラブの戦力に入っている」と言える序列**（＝走れる人数の2倍。7区間なら14番手まで）。
 * 故障者が出れば少し下まで回ってくるので、ここまでが戦力・ここから下が余剰。
 *
 * **出す側と買う側で、同じこの線を使うこと。** 以前は
 *
 *   出す側（`transferDecision.hasNoPlayingTime`）… 15番手以降なら余剰＝出してよい
 *   買う側（`squadNeeds.needsPlayer`）          … 走れる**7人**に入らないなら要らない
 *
 * と食い違っていました。**8〜14番手の選手は「誰もが売るが誰も買わない」**ことになり、
 * OVR70以下の1,208人（全体の21%）が232クラブ全部から「要らない」と判定されて
 * 市場から丸ごと消えていました（3年で移籍1件）。線を揃えると格15〜20のクラブが
 * 受け入れるようになります（OVR64は依然0クラブ＝**相対なので緩めすぎにはならない**。
 * 14番手のOVRは格1で83.2・格20で65.8）。
 */
export const SQUAD_DEPTH_SLOTS = RUNNING_SLOTS * 2
