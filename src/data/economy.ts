 ...
 *
 * ★**上限は手元の資金だけです**（オーナー・2026-08-21「上限撤廃」）。
 *   以前は「格の年間予算の20%まで」（`TRANSFER_BUDGET_SHARE`）という蓋があり、
 *   格1でも4.22億が天井でした。OVR93・27歳・残3年の移籍金は4.07億なので、
 *   **払えるのは世界で格1の5クラブだけ**（格2は4.02億でもう届かない）。
 *   そこへ「海外挑戦に登録した選手は憧れの地域のクラブしか声を掛けられない」が
 *   重なって、**世界一の選手に1件も打診が来ない**状態になっていました。
 *
 * ★**割合の蓋を戻さないこと。** 格の差は年間予算（`clubTier.TIER_BUDGET`）が
 *   そのまま資金の増え方に効くので、蓋が無くても格1と格20では貯まる額が違います。
 *   蓋は「格の差」を二重に掛けたうえで、上位を一律の天井で揃えてしまう形でした。
 *
 * ★それでも**この関数を通すこと**。以前この式が store の中に3通り書かれていて、
 *   ・上乗せの判定は年間予算だけ見て手元の資金を見ていない
 *   ・打診の生成は手元の資金だけ見て年間予算の上限を見ていない
 *   ・横取りの判定だけが正しい
 *   という状態でした。海外クラブの打診にいたっては上限が一つも無かった。
 *
 * @param cash 手元の資金（`finance.budget`）
 */
export function transferCapOf(cash: number): number {
