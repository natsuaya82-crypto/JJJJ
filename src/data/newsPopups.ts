...
export const NEWS_POPUPS: NewsPopup[] = [
  // ★**いま出しているお知らせはありません。**
  //   ランクマッチ中止の案内（`rated-cancel-2026-09`）は、ランクマッチそのものを
  //   隠したので一緒に消しました（2026-09-18）。**機能を隠したのに案内だけ残すと、
  //   押しても行き先が無いお知らせになります**（`to` は `/online/rated` でした）。
  //   足すときは `from` / `until` を必ず付けること（期間ものは `repeat` も）。
]
