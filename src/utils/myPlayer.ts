 ...
 *   不得意へ99を振ればその99は消えず、**そこは伸びないだけ**になります。
 *   つまり上限より上へ振るのは「伸びしろを捨てて今の数値を取る」選択です。
 *
 * ★**振り分けを型の上限で止めないこと**（オーナー・2026-08-22）。
 *   エースが速力89（型の上限87）に振れて、そのまま89で止まる形が正。
 *   **そのぶん平均は92を超えます**（実測：エースが不得意へ盛ると平均95.4、
 *   560点なら97.1）。これは承知のうえの仕様で、**クランプを足さないこと**。
 *   `check-my-player` の⑤が「不得意へ振った99が消えないこと」を見ています。
 */
