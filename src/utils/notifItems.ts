...
export function collectNotifications(input: NotifInput) {
  ...
  // ★**買い取りの打診はベルにも通知ページにも出さない**（2026-08-12・オーナー判断）。
  //   「受信箱はいいけど通知に来なければいい」——1レースに1〜2件しか来ないのに
  //   打診は5レース残るので、通知に出すと**常時8〜11件**が並び続けていた。
  //   返事は**移籍ページ**（`TransferPage` の「他クラブからのオファー N件 — 要確認」）と
  //   **チャット**でできるので、通知から外しても詰まらない。
  //   ★`incomingOfferPlayers` は返り値には残す（移籍ページが読む）。**数えないだけ**。
  const total = 0
    + stayOrLeave.length
    + tradeOffers.length
