 ...
 * 格 → 年間予算（円）。
 *
 * もとの決め方は「その格のロスター構成を年齢18〜32でばらけさせたときの総年俸 ÷ 0.6」＝
 * 初期ロスターの年俸が予算の6割。**その値を全体で 1.10 倍したのがいまの表です**
 * （オーナー・2026-09-18「人件費は見直していい感じに…数値全体でいじって調整」）。
 * ★格ごとの比は1つも変えていません（21.1億→4.2億の開きは、いまの 23.2億→4.6億 でもそのまま）。
 *
 * ■なぜ上げたのか（実測・10年・同じ種）
 *   予算は格で固定なのに、**世界のOVRは10年で 77.3 → 80.8 に上がります**。
 *   年俸は `faMarketSalary`＝OVRから出るので、人件費だけが毎年ふくらむ。
 *   実測で資金の中央値は5年目に 11.45億 まで増えたあと下り坂に入り、
 *   6年目に初めてマイナスのクラブが出て、10年目に 8.06億 ／ 5クラブがマイナス
 *   （うち2クラブは下限の −1.00億 に張り付き）。
 *   年俸 ÷ 予算の中央値は**年0で55%、年10で国内77%・海外68%**（3部は92%・最大104%）。
 *   支出は「年俸×1.1 ＋ 施設維持費（予算の約24%）」なので、**69% で収支が均衡**します。
 *   つまり10年目には過半のクラブが赤字を貯金で埋めている状態でした。
 * ★**数字を動かすときは年0と年10の両方を測ること。** 片方だけ見ると、
 *   年0を直すと年10が破綻し、年10を直すと年0が緩みます。
 */
 ...
 *
 * ■**格1=7.0 → 格20=2.8 の等差**（オーナー・2026-08-16「自チームはカードあるぶん
 *   簡単に作れるから、他チームは成長速度上げるか。格によって成長する速度を上げよう」
 *   →「7でよろ」）
 *
 *   以前は 3.0 → 1.65 で、**格11〜20が全部1.5の横並び**でした（下位10段は
 *   格が違っても育ち方がまったく同じ）。全段に傾斜を付けて上げてあります。
 *
 *   ■**自チームには効きません。** 自チームは毎レースの一律EXP＋カード＋施設で育てる
 *     ので、この倍率はCPU・海外だけ。「自チームはカードがあるぶん、他チームは倍率で
 *     埋める」がこの表の役目です。`ANNUAL_BASE_EXP` を上げると自チームまで速くなる
 *     ので、他チームだけ動かしたいときは**必ずこちら**を触ること。
 *
 *   実測（格1のSSS・普通型を18歳から育てた到達点 ／ 世界を12年回した OVR92+ / 90+ / 85+）
 *
 *     倍率        27歳  30歳    92+ / 90+ / 85+
 *     世界を作った直後     —      11 /  96 /  715
 *     3.0→1.5（前）  —    —       4 /  28 /  509   ← 上位が枯れる
 *     5.0→2.0       90   92       9 /  79 / 1054
 *     **7.0→2.8**   92   93      30 / 216 / 1422   ← これ
 *     8.0→3.2       92   93      73 / 265 / 1580
 *     11.0→4.4      94   94     173 / 456 / 1939
 *     14.0→5.6      95   95     263 / 626 / 2142   ← 初期の15倍。インフレ
 *
 *   14.0 は `ANNUAL_BASE_EXP` の設計値（格1のSSSが27歳で95）にちょうど届きますが、
 *   **同じ倍率が全クラブに掛かる**ので世界ごと持ち上がります。7.0 は格10・格20を
 *   ほとんど動かさずに（+0.7 / +1.1）上位だけ厚くなる値。
 *
 * ★**ここは「格が高いほど速い」の唯一の表です。** 倍率を他所に書かないこと。
 *   数を変えるときは、上と同じやり方（格1のSSSの到達点と、世界を12年回した
 *   OVR92+/90+/85+）を両方数えてから決めること。
 *   ★**`growPlayer` は中で加齢します**（`age: nextAge` を返す）。手で追うときに
 *     呼ぶ側で `age + 1` を足さないこと——1回で2歳進んで到達点を6〜7点低く読みます。
 */
const TIER_GROWTH_RATE: Record<ClubTier, number> = {
  1: 7.0, 2: 6.78, 3: 6.56, 4: 6.34, 5: 6.12, 6: 5.89, 7: 5.67, 8: 5.45, 9: 5.23, 10: 5.01,
  11: 4.79, 12: 4.57, 13: 4.35, 14: 4.13, 15: 3.91, 16: 3.68, 17: 3.46, 18: 3.24, 19: 3.02, 20: 2.8,
}
...
// ── 海外（9リーグ×20クラブ）─────────────────────────────────────
//
// ★**海外クラブの格は動かない**（初期値のまま一生固定・オーナー・2026-08-18「格はもう動かさない。国内だけ動かす」）。
//   以前はここに「海外の格も毎年動く」と書いてあったが、それは `tierFromForeignRank`（廃止）のころの話。
//
// リーグごとに帯があり、初期値はその中に配る。リーグ同士の入れ替えは無いので、
...
/**
 * 帯の中の何番目か（0..1）から格を出す。**初期値を作るときだけ通る**（海外の格は動かないので更新は無い）。
 ...
 * 無所属は undefined を返す。
 *
 * clubs には世界のクラブ（`GameState.clubs`＝国内＋海外の1つの並び）をそのまま渡すこと。
 * 渡さなかったクラブは data/clubTiers.ts の初期値になる（＝毎年動いたぶんが反映されない）。
 */
...
function tieredIndex(tiered: readonly TieredTeam[]): Map<string, TieredTeam> {
  const hit = clubIndexCache.get(tiered)
  if (hit && hit.len === tiered.length) return hit.index
  const index = new Map<string, TieredTeam>()
  // find と同じく**最初に見つかったもの**を返す（同じIDが2つあっても答えを変えない）。
  // id の無いクラブは find でも当たらないので入れない
  for (const c of tiered) if (c.id != null && !index.has(c.id)) index.set(c.id, c)
  clubIndexCache.set(tiered, { len: tiered.length, index })
  return index
}
...
export function tierOfPlayerClub(
  ...
  const t = clubs && tieredIndex(clubs).get(teamId)
