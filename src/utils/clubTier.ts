 ...
 *
 * ■**格1=5.0 → 格20=2.0 の等差**（オーナー・2026-08-16「自チームはカードあるぶん
 *   簡単に作れるから、他チームは成長速度上げるか。格によって成長する速度を上げよう」）
 *
 *   以前は 3.0 → 1.65 で、**格11〜20が全部1.5の横並び**でした（下位10段は
 *   格が違っても育ち方がまったく同じ）。全段に傾斜を付けて上げてあります。
 *
 *   同じ世界を12年回した実測（`OVR90+ / OVR85+ の人数`）
 *
 *     世界を作った直後      107 / 702
 *     3.0→1.5（前）          28 / 509   ← 上位が枯れる
 *     4.0→1.5                41 / 838
 *     **5.0→2.0（これ）**    83 / 1113  ← 初期の世界にいちばん近い
 *     6.5→2.5               169 / 1374  ← 初期の1.6倍。インフレ
 *
 * ★**ここは「格が高いほど速い」の唯一の表です。** 倍率を他所に書かないこと。
 *   数を変えるときは、上と同じやり方（世界を12年回してOVR90+/85+を数える）で
 *   確かめてから決めること。
 */
const TIER_GROWTH_RATE: Record<ClubTier, number> = {
  1: 5.0, 2: 4.84, 3: 4.68, 4: 4.53, 5: 4.37, 6: 4.21, 7: 4.05, 8: 3.89, 9: 3.74, 10: 3.58,
  11: 3.42, 12: 3.26, 13: 3.11, 14: 2.95, 15: 2.79, 16: 2.63, 17: 2.47, 18: 2.32, 19: 2.16, 20: 2.0,
}
