 ...
 *
 * 選手IDから決まるので毎シーズンぶれない（30〜36でばらつき、実力者は少し長く現役）。
 *
 * ■**32〜40 から 30〜36 へ下げました**（オーナー・2026-08-16「30〜36」）
 *   ピークは 22／27／30（`engine/ageCurve.ts` の `PEAK_AGE`）なのに引退の平均が36で、
 *   **ピークを過ぎてから9年も居座って**いました。同じ世界を12年回すと年齢の分布が
 *
 *     0年目  〜20 25% / 21-23 25% / 24-26 27% / 27-29 19% / 30-32  4% / 33〜  0%
 *     12年目 〜20  7% / 21-23 18% / 24-26 15% / 27-29 10% / 30-32 28% / 33〜 22%
 *
 *   と、**世界の半分が30歳以上**になります（オーナー「年々弱くなるのを改善してほしい」）。
 *
 * ■なぜ1本にしたのか
 *   まったく同じ式が gameStore の2か所に、**ハッシュ関数ごと**書いてあった。
 *   片方は最終戦後の「引退表明」のニュース、もう片方は開幕時の**実際に引退させる処理**で、
 *   コメントに「同じ決定式を1歳先で評価」と書いて人が手で合わせていた。
 *   30 や 5 を片方だけ触ると「引退しますと言ったのに翌年も走っている」が出る。
 */
export const RETIRE_AGE_MIN = 30
export const RETIRE_AGE_MAX = 36
export function retirementAgeOf(p: Player): number {
  const o = ovr(p)
  const bonus = o >= 80 ? 2 : o >= 72 ? 1 : 0
  // 実力者ボーナス(最大+2)を足しても MAX を超えない幅で振る
  return Math.min(RETIRE_AGE_MAX, RETIRE_AGE_MIN + (strHash(p.id) % 5) + bonus)
}

/**
 * **その選手が引退する歳に達しているか。引退の可否はこの1本だけ。**
 *
 * ■なぜ要るのか（オーナー・2026-09-15「引退一本化してないのはなぜ？
 *   そもそも人によって違うとかおかしいよね」）
 *   年齢の式（`retirementAgeOf`）は1本でしたが、**それを呼ぶ側が3つあって、
 *   3つとも条件が違いました。**
 *
 *   | どこ | 式 | 誰を見ていたか |
 *   |---|---|---|
 *   | `engine/retirement.ts` | `age >= retirementAgeOf` | クラブ所属の人だけ（**FAは対象外**） |
 *   | `engine/seasonFinaleNews.ts` | `(age + 1) >= retirementAgeOf` | **国内クラブだけ**（海外の引退は一度もニュースにならない） |
 *   | `engine/contractRequests.ts` | **`age >= 35` の直書き** | 自チームだけ |
 *
 *   3つ目は `retirementAgeOf` を呼んでいないので、**31歳で引退する選手には
 *   引退の打診が一生来ず、36歳の選手には35歳から来ます。**
 *
 * ■★**除外を付けないこと。** 誰であっても歳が来たら引退します。
 *   1つ目の「FAは対象外」と「今季契約が切れる人は対象外」が噛み合って、
 *   **不死の選手**ができていました——36歳で契約が切れると満了が優先されてFAになり、
 *   FAは引退の対象外なので、そのまま歳を取り続ける。実測で10年目に38歳
 *   （`RETIRE_AGE_MAX` は36）。除外はどれも書いた当時の事情で、その事情が
 *   消えたあとも除外だけ残っていました。
 *
 * @param aheadYears 何年先で見るか。0＝今季の終わりに引退する人、
 *                   1＝来季の終わりに引退する人（引退表明のニュースと、本人が
 *                   引退を言い出すかの打診が使う）。**別々の式を書かないこと。**
 */
export function isRetiringAge(p: Player, aheadYears = 0): boolean {
  return (p.age + aheadYears) >= retirementAgeOf(p)
}

/**
 * 移籍金の年齢倍率。移籍金＝市場年俸×これ。若いほど高く、伸びしろの値段になる。
 *
 * ■段をやめました（2026-08-11・オーナー判断）
 *   以前は 〜22歳×5／23〜27×4／28〜31×3／32〜×2 の**4段**でした。
 *   能力が1も変わっていないのに誕生日をまたいだ瞬間に −20%／−25%／−33% と落ちるので、
 *   「27歳の主力は28歳になる前に売る」が最適解になっていました（`docs/BACKLOG.md` A-1）。
 *
 *   いまは**折れ線で連続**です。**もとの4段の帯の中央を通します**（21歳=5／25歳=4／
 *   29.5歳=3／34歳=2）。中央を通すので帯ごとの平均はほぼそのままで、
 *   1年あたりの下落は最大でも6%程度に収まります。
 *
 *     18〜21歳 5.00      26歳 3.78      31歳 2.67
 *         22歳 4.75      27歳 3.56      32歳 2.44
 *         23歳 4.50      28歳 3.33      33歳 2.22
 *         24歳 4.25      29歳 3.11      34歳〜 2.00
 *         25歳 4.00      30歳 2.89
 *
 *   ★35歳以降が2.00で止まるのは以前と同じ（下限）。
 *   ★点をいじるときは `AGE_FEE_POINTS` だけを触ること。式は他所に書かない。
 */
