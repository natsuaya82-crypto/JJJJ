...
const IND_ANCHORS: Record<number, [number, number][]> = {
  // ★**110 は優勝トロフィーでしか届かない。** 勾配は 90→99 と同じ（駅伝の PACE_TABLE と揃える）
  5000:  [[50, 870], [70, 825], [90, 780], [99, 750], [110, 713]],
  10000: [[50, 1770], [70, 1695], [90, 1620], [99, 1560], [110, 1487]],
  21097: [[50, 3900], [70, 3720], [90, 3540], [99, 3420], [110, 3273]],
  42195: [[50, 7980], [70, 7740], [90, 7470], [99, 7200], [110, 6870]],  // 90は2:04:30（日本記録級が量産されない傾きに）。99=世界記録レベルは設計通り
}
...
export function individualBaseTime(o: number, distance: 5000 | 10000 | 21097 | 42195): number {
  // ★上端は表がクランプするので、ここで 99 に丸めない（トロフィーで 99 超えの選手がいる）
  return lerpAnchors(IND_ANCHORS[distance], o, { belowFirst: 'extend' })
}
