...
import type { Nationality, Player, Race, SeasonAward } from '../types'
...
export function computeSeasonAwards(races: Race[], players: Player[], year: number): SeasonAward {
  ...
  return awardsFromStats(seasonStats(races), players, year, id => byId.get(id)?.name)
}
...
/** 過去シーズンから必要な物だけを受ける */
export type SeasonRacesLike = { year: number; races?: Race[] }

type StatsByYear = { year: number; stats: SeasonStats }[]
...
function statsByYear(pastSeasons: SeasonRacesLike[]): StatsByYear {
  ...
  const value = pastSeasons
    .filter(Boolean)
    .map(s => ({ year: s.year, stats: seasonStats(s.races ?? []) }))
...
export function seasonAwardsOf(
  ...
  const value = byYear.map(({ year, stats }) => awardsFromStats(stats, players, year, nameOf))
